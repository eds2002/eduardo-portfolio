      ...
      <div className="mx-auto grid max-w-2xl grid-cols-1 gap-8 overflow-hidden lg:mx-0 lg:max-w-none lg:grid-cols-1">
        {timeline.map((item) => (
          <Link
            ...
            target="_blank"
            key={item.name}
            className="hover:bg-primary-1/15 rounded-2xl p-3 transition-colors cursor-pointer "
          >
            <div className="flex items-center text-sm font-medium leading-6 text-primary-1">
              <svg
                viewBox="0 0 4 4"
                className="mr-4 h-1 w-1 flex-none"
                aria-hidden="true"
              >
                <circle cx={2} cy={2} r={2} fill="fill-primary-1" />
              </svg>
              {item.date}
              <div
                className="absolute -ml-2 h-px w-screen -translate-x-full bg-primary-1 sm:-ml-4 lg:static lg:-mr-6 lg:ml-8 lg:w-auto lg:flex-auto lg:translate-x-0"
                aria-hidden="true"
              />
            </div>
            <div className="gap-3 flex flex-col">
              ...
              <ul className="px-4 flex flex-col gap-1">
                {item.responsibility?.map((responsibility) => (
                  <li className="list-disc " key={responsibility}>
                    <Typography
                      variant="p"
                      affects={'removePMargin'}
                      className="text-sm opacity-75"
              ...
              <div className="flex flex-row gap-1 flex-wrap">
                {item.stack.map((tech) => (
                  <span
                    key={tech}
                    className="text-sm text-primary-1 font-medium px-4 py-1 bg-primary-1/15 rounded-full block"
                  >
                    {tech}
                  </span>
                ))}
        ...
        <Link
          href={pdfLink}
          className="font-medium text-md text-primary-1 hover:text-primary-4 group flex items-center bg-transparent self-start justify-self-start"
          target="_blank"
        >
